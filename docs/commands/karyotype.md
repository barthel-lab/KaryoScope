...
| `--no-legend` | Don't draw the color legend in the right margin of the SVG. The legend lists only features actually drawn, collapsed into groups when the database's `colors.tsv` declares a `legend_group` column — see [The legend](#the-legend). |
...
The legend in the right margin lists the features actually drawn — not every
feature in the database, and not features too small to see. A feature whose
entire drawn extent is under half a pixel is omitted, since a legend row for it
would only send the reader hunting for a colour that was never visibly rendered.

When the database's `colors.tsv` carries the optional 4th `legend_group`
column, features sharing a group collapse to a single row labelled by the
group. That is what makes a large feature set legible: the CHM13 cytoband
database has 833 features in 9 colours, and without grouping the legend
silently truncated to the ~51 rows that fit the canvas — showing 6% of the
features present with no indication the rest were dropped.

Databases without the column keep a per-feature legend, unchanged. See
[`build` → Grouping the legend](build.md#grouping-the-legend) for how to
declare groups when building a database.
