...
| `--combine-acrocentrics / --no-combine-acrocentrics` | Also combine acrocentric chromosomes when `--combine-chromosomes` is set. Off by default. **[default: no-combine-acrocentrics]** |
