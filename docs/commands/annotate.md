...
For human-scale inputs, use at least 16 threads. Memory to request depends on the backend and input shape. With the **HKS backend**, the peak is set by the index rather than the input — the shared base index plus one feature set's labeling at a time — so it is ~10 GB whether you annotate a single haplotype or a combined diploid assembly such as HG002 v1.1 (request ≥ 16 GB). The **KMC backend**'s k-mer query peaks at ~36 GB on a haplotype and ~46 GB on a combined diploid, and the smoothing worker pool that follows counts toward the same allocation under cgroup accounting — a 64 GB SLURM request has been OOM-killed during smoothing at `-t 16` — so request 128 GB or reduce `--threads`. Runtimes at `-t 16`, medians of three: a haplotype takes ~7–9 min (HKS) or ~13–14 min (KMC); HG002 v1.1 combined takes ~14–16 min (HKS) or ~23–27 min (KMC). For the machine these were measured on, see the README's [System requirements](../../README.md#system-requirements).
